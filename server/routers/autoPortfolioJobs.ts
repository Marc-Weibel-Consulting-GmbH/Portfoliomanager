        ...
        try {
          ...
          const computeAdjustedPositions = (base: typeof positions, finalAdjustments: any[]) => {
            ...
            if (replaceAdj.length > 0) {
              ...
              for (const ra of replaceAdj) { const idx = adjusted.findIndex(p => p.ticker.toUpperCase() === ra.ticker.toUpperCase()); if (idx < 0 || isSleevePos(adjusted[idx])) continue; const replacement = candidates.shift(); if (!replacement) continue; usedTickers.add(replacement.stock.ticker.toUpperCase()); adjusted[idx] = { ...adjusted[idx], ticker: replacement.stock.ticker, companyName: replacement.stock.companyName, sector: replacement.stock.sector, currency: replacement.stock.currency, currentPrice: parseFloat(String(replacement.stock.currentPrice ?? '0')) || 0, combinedScore: replacement.combinedScore, signal: replacement.signal, reason: `Ersetzt ${ra.ticker} gemäss KI-Empfehlung`, assetType: 'stock' as const, assetClass: undefined }; (adjusted[idx] as any).aiReason = undefined; }
            }
