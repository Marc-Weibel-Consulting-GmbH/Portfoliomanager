...
// Generate signals based on RSI + MACD strategy
function generateSignals(
  ...
  const startIdx = Math.max(rsiOffset, macdOffset) + 1;
  ...
  for (let i = startIdx; i < prices.length; i++) {
    ...
    // Buy signal: RSI < 35 AND MACD crosses above signal
    if (currentRSI < 35 && macdCrossUp) {
      signals.push({
        date: dates[i],
        ticker,
        type: "buy",
        reason: `RSI überverkauft (${currentRSI.toFixed(1)}) + MACD Kreuzung aufwärts`,
        price: prices[i],
      });
    }
    // Sell signal: RSI > 70 AND MACD crosses below signal
    else if (currentRSI > 70 && macdCrossDown) {
      signals.push({
        date: dates[i],
        ticker,
        type: "sell",
        reason: `RSI überkauft (${currentRSI.toFixed(1)}) + MACD Kreuzung abwärts`,
        price: prices[i],
      });
    }
    // Softer buy: RSI < 30 (strong oversold)
    else if (currentRSI < 30 && rsi[rsiIdx - 1] >= 30) {
      signals.push({
        date: dates[i],
        ticker,
        type: "buy",
        reason: `RSI stark überverkauft (${currentRSI.toFixed(1)})`,
        price: prices[i],
      });
    }
    // Softer sell: RSI > 75 (strong overbought)
    else if (currentRSI > 75 && rsi[rsiIdx - 1] <= 75) {
      signals.push({
        date: dates[i],
        ticker,
        type: "sell",
        reason: `RSI stark überkauft (${currentRSI.toFixed(1)})`,
        price: prices[i],
      });
    }
  }
...
export const backtestRouter = router({
  ...
  runSingle: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        const signals = generateSignals(validDates, prices, input.ticker);
  ...
  runPortfolio: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      for (const stock of stocks.slice(0, 20)) { // Limit to 20 stocks for performance
        try {
          ...
          const signals = generateSignals(validDates, prices, stock.ticker);
          const trades = signalsToTrades(signals);
