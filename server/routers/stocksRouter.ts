...
export const stocksRouter = router({
  ...
  /**
   * Phase 1 der Universum-Zusammenführung (STOCK_UNIVERSE_MERGE.md):
   * Kopiert die kuratierten `watchlistStocks` per Ticker-Upsert in `stocks`.
   * Idempotent — mehrfaches Ausführen ist gefahrlos.
   *  - Neue Ticker werden angelegt (mit Kuratierungs- und Kennzahlfeldern).
   *  - Bestehende Ticker: NUR die Kuratierungs-Facette (listType, source,
   *    signalScore/Type, aiReason, rsi14, industry, country, notes, isActive,
   *    lastMetricsUpdate) wird gefüllt — Portfolio-Stammdaten (Name, Sektor,
   *    portfolioWeight, Moats …) bleiben unangetastet.
   */
  mergeWatchlistUniverse: adminProcedure.mutation(async () => {
    const { getDb } = await import("../db");
    const { stocks, watchlistStocks } = await import("../../drizzle/schema");
    const { eq } = await import("drizzle-orm");
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Datenbank nicht verfügbar" });

    const wl = await db.select().from(watchlistStocks);
    const existing = await db.select({ ticker: stocks.ticker }).from(stocks);
    ...
    for (const w of wl as any[]) {
      const curation = {
        listType: w.listType,
        source: w.source,
        signalScore: w.signalScore ?? null,
        signalType: w.signalType ?? null,
        aiReason: w.aiReason ?? null,
        rsi14: w.rsi14 ?? null,
        industry: w.industry ?? null,
        country: w.country ?? null,
        notes: w.notes ?? null,
        isActive: w.isActive ?? 1,
        lastMetricsUpdate: w.lastMetricsUpdate ?? null,
      };
      if (existingSet.has(w.ticker)) {
        await db.update(stocks).set(curation).where(eq(stocks.ticker, w.ticker));
        updated++;
      } else {
        await db.insert(stocks).values({
          ...
          marketCap: w.marketCap ?? null,
          currentPrice: w.currentPrice ?? null,
          peRatio: w.peRatio ?? null,
          pegRatio: w.pegRatio ?? null,
          dividendYield: w.dividendYield ?? null,
          beta: w.beta ?? null,
          week52High: w.week52High ?? null,
          week52Low: w.week52Low ?? null,
          ...curation,
        });
        inserted++;
      }
    }
    return { inserted, updated, total: wl.length, message: `Universum zusammengeführt: ${inserted} neu, ${updated} aktualisiert (${wl.length} Watchlist-Titel).` };
  }),
});
