...
import { CheckCircle2, Clock3, ExternalLink, Eye, FileSearch, Loader2, ShieldCheck, XCircle } from "lucide-react";
...
export default function AdminResearchDesk() {
  ...
  const runNow = trpc.researchDesk.runShadowNow.useMutation({
    onSuccess: async (result) => {
      await utils.researchDesk.overview.invalidate();
  ...
  const setCheckerStatus = trpc.researchDesk.setCheckerStatus.useMutation({
    onSuccess: () => void utils.researchDesk.overview.invalidate(),
