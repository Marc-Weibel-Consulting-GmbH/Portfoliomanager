...
| S2-05 | Security | Fehlende Standard-HTTP-Schutzheader | verifiziert | Express-Fingerprinting deaktiviert; MIME-Sniffing, Clickjacking, Referrerweitergabe und Geräteberechtigungen mit getesteten Headern begrenzt. HSTS ist produktiv aktiv. CSP bleibt bis zur Ressourceninventur explizit offen. |
| S2-06 | Security | Globales Transaktions-Auditlog | verifiziert behoben | Anonymer Abruf gab reale, globale Änderungsprotokolle aus. Da keine Nutzer- oder Portfoliozuordnung existiert, sind Abruf und globale Löschung jetzt fail-fast auf Administratoren begrenzt; anonyme und reguläre Nutzer testen grün gegen `UNAUTHORIZED` beziehungsweise `FORBIDDEN`. |
| S2-07 | Security | Öffentliche TradingView-Analysebrücke | verifiziert behoben | 17 kosten- und lastintensive externe Analyseprozeduren verlangen nun eine gültige Sitzung. Routertest belegt, dass anonyme `status`- und `stockScoring`-Aufrufe vor MCP-Initialisierung scheitern. |
| S2-08 | Security | Secret-Metadaten in Boot-Logs | verifiziert behoben | Startdiagnostik mit Schlüsselpräfixen und -längen entfernt. Die explizite Verfügbarkeitsdiagnostik enthält nur Booleans; Test schliesst Wert-, Präfix- und Längenlecks aus. |
| S2-09 | Teststabilität | Ungesteuerte Live-Checks und ungültige ISIN-Fallbacks | verifiziert behoben | Perplexity folgt nun dem projektweiten expliziten Live-Opt-in. Ungültige Identifikatoren werden vor Yahoo-/EODHD-Netzaufrufen verworfen; die bisher timeout-behaftete Negativprobe ist deterministisch. |
| S2-10 | Security | Benchmark-Seed nur mit Inline-Rollencheck | verifiziert behoben | Zentraler `adminProcedure` ersetzt individuellen Laufzeitcheck; anonyme und reguläre Nutzer werden vor der DB-Operation abgewiesen. |
| S2-11 | Security & Privacy | Öffentliche LLM- und Formularpfade | verifiziert behoben | LLM-Tagesnews setzt Authentisierung voraus. Newsletter und Kontakt validieren und begrenzen Eingaben; Kontakt-PII wird nicht mehr geloggt. |
| S2-12 | Dependencies | Direkte und erreichbare Produktionsbibliotheken | bewertet und teilweise behoben | tRPC, Axios, Drizzle, Officeparser, Nanoid, Nodemailer aktualisiert und SheetJS durch ExcelJS ersetzt. Audit: 0 kritisch, 22 hoch, 17 moderat; Restpfade und Lizenz-Review als Release-Gates dokumentiert. |
| S2-13 | Dependencies | Erreichbare Mail-, HTML- und Messaging-Pfade | verifiziert behoben | Nodemailer 9, jsdom 30 und Twilio 6 aktualisiert. Build, TypeScript, Wikifolio- und WhatsApp-Regressionen bestanden; Audit: 0 kritisch, 19 hoch, 9 moderat. |
| S2-14 | Governance | Portfolio-Metrics ohne Heartbeat-Bindung | verifiziert behoben | Heartbeat `portfolio-metrics-snapshot-daily` täglich 14:30 UTC registriert; UID `mmrK8Eey5oq7PiuuAQu9Kn` ist persistent auf den cron-authentisierten Handler gebunden. Kontrollierter autorisierter Handlerlauf: HTTP 200, 95 Portfolios, 0 Fehler. |
