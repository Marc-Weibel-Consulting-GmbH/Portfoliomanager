...
| F-16 | **Live-/Intraday-Kurse für Indizes und Tagesveränderung** (Frage Auftraggeber, 2026-07-03: TradingView-MCP als Quelle?). Einordnung: Der TradingView-MCP ist ein externer Railway-Microservice auf inoffizieller Basis (Screener-Scraping) — gut für Signale/TA, aber als Quelle für die **zentralste Zahl der App** (Depotwert/Tagesveränderung) riskant: keine SLA, Symbol-Mapping (SIX:NESN vs. NESN.SW), Ausfall = falsche oder fehlende Kernzahlen; die MCP-Tests laufen nur mit gesetzter `TRADINGVIEW_MCP_URL`. **Empfehlung:** Live-Kurse aus dem vorhandenen **EODHD-Vollplan** beziehen (Real-Time/Delayed-Quote-Endpoint ist im All-in-One-Plan enthalten, offizielle API, bereits integriert — konsistent mit D-14 «ein Kursanbieter»): (a) Indizes auf der Marktseite: echte Index-/Spot-Ticker via EODHD (löst R-35 gleich mit); (b) Tagesveränderung: Basis bleibt der symmetrische R-29-Fix (Schlusskurs vs. Vortagesschluss aus einer Quelle); darauf optional ein Intraday-Layer «Live-Kurs vs. letzter Schluss» aus EODHD, klar als «Intraday» gelabelt und mit Fallback auf die Schlusskurs-Rechnung bei API-Ausfall. TradingView-MCP allenfalls als sekundärer Fallback, nicht als Primärquelle. *Hinweis: die neu hinzugekommene `stocks.getDailyPerformance`-Prozedur (`stocksRouter.ts:15–89`) hat dasselbe R-29-Muster (currentPrice vs. historischer Schluss, Fallback `yesterdayPrice = currentPrice`, gewichts- statt stückzahlbasiert) — beim R-29-Fix mitziehen.* | Vorgabe/Frage; `server/routers/stocksRouter.ts:15–89`, `server/routers/marketRegimeRouter.ts`, `server/_core/eodhdApi.ts` | M |
